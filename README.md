...
| 16 | Στον πίνακα `Hospitalization` αντικαταστήσαμε το σύνθετο PK `(PatientAMKA, AdmissionDateTime)` με surrogate `HospitalizationID INT AUTO_INCREMENT`. Το σύνθετο κλειδί παρέμεινε ως `UNIQUE`. | Το σύνθετο PK (char(11) + datetime = 19 bytes) διαδίδεται ως FK σε πίνακες όπως `TriageEvent`, `HospEvaluation`, `DoctorEvaluation`, `ExitDiagnosis`, `AdmissionDiagnosis` και `HospLabTest`, δημιουργώντας σύνθετα keys και φουσκώνοντας σημαντικά τα B+ trees τους. Με surrogate `INT` οι αναφορές γίνονται μονής στήλης. |
| 17 | Στον πίνακα `Shift` αντικαταστήσαμε το τριπλό σύνθετο PK `(DepartmentID, ShiftTypeName, Date)` με surrogate `ShiftID INT AUTO_INCREMENT`. Το σύνθετο κλειδί παρέμεινε ως `UNIQUE`. | Το τριπλό PK διαδίδεται στους `hasDoctor`, `hasNurse`, `hasAdmin`, κάνοντάς τους να έχουν 4-column composite PKs. Με surrogate `ShiftID` τα PKs τους γίνονται `(ShiftID, PersonAMKA)`. |
| 18 | Κάποιοι επιχειρησιακοί περιορισμοί που λογικά εκφράζονται με `CHECK` υλοποιήθηκαν με `TRIGGER`, επειδή στη MySQL δεν επιτρέπεται σε ορισμένες περιπτώσεις η χρήση `CHECK` πάνω σε κλειδιά που συμμετέχουν σε foreign keys με referential actions. Συγκεκριμένα, μέσω triggers επιβάλλουμε: (a) ένας γιατρός δεν μπορεί να επιβλέπει τον εαυτό του, (b) συνέπεια μεταξύ `Doctor.Rank` και `SupervisorAMKA` (`Resident` απαιτεί μη-NULL supervisor, `Director` απαιτεί NULL supervisor), (c) κάθε εγγραφή `Image` συνδέεται με ακριβώς μία οντότητα από `Staff`, `Department`, `ProcedureRoom`, `Room`. | Έτσι οι κανόνες αυτοί ελέγχονται αξιόπιστα σε `INSERT/UPDATE` χωρίς σύγκρουση με περιορισμούς της μηχανής γύρω από `CHECK` και referential actions. |
| 19 | Επιτρέπουμε updates στο `Staff.AMKA` (το πρωτεύον κλειδί του πίνακα `Staff`) για να μπορούμε να διορθώσουμε λάθη που προέκυψαν κατά την αρχική εισαγωγή ή πληκτρολόγηση. Χάρη στο `ON UPDATE CASCADE` που ορίστηκε στα foreign keys που αναφέρονται στο `Staff.AMKA` (σε `Doctor`, `Nurse`, `AdminStaff`, `StaffPhone`), όλες οι συσχετισμένες εγγραφές ενημερώνονται αυτόματα. Επιπλέον, τα triggers που έχουμε δημιουργήσει (`trg_doctor_staff_type_check_update`, `trg_nurse_staff_type_check_update`, `trg_adminstaff_staff_type_check_update`) εγγυώνται ότι η συνέπεια μεταξύ του τύπου του staff και των subtypes διατηρείται σε κάθε ενημέρωση. | Αυτή η προσέγγιση επιτρέπει τη διόρθωση δεδομένων χωρίς να χαθούν ιστορικά στοιχεία, ενώ ταυτόχρονα διασφαλίζει την ακεραιότητα των αναφορών δεδομένων σε ολόκληρο το σχήμα. |
...
| 21 | The `ProcedureEvent.MainDocAMKA` represents the single primary doctor who must be exclusively assigned to and remain responsible for the entire duration of that `ProcedureEvent` (i.e., may not be the main doctor of any overlapping `ProcedureEvent`). Other participating staff (`operates_in`, `assists_in`, `helps_in`) may assist in multiple procedure events, including overlapping ones. | Clarifies primary-surgeon exclusivity while allowing flexible assistant participation. |
| 22 | We distinguish emergency arrivals from direct scheduled admissions. Emergency arrivals always create a `TriageEvent` and enter the priority FIFO `PatientQueue`, while scheduled/direct admissions may register the patient and create a `Hospitalization` without triage. | This keeps the assignment's emergency workflow intact while supporting planned hospital admissions that are not emergency-department cases. |
...
| 6 | For doctor-evaluation analytics, doctor-specific scores are stored in `DoctorEvaluation` and hospital-level scores are stored in `HospEvaluation`. A `DoctorEvaluation` row is allowed only when the doctor prescribed medication during that hospitalization. | This keeps general hospitalization feedback separate from doctor-specific feedback and enforces the assignment rule that patients evaluate only doctors involved in prescription during their hospitalization. |
...
> Several of our queries revolve around counting the number of procedures a specific doctor (**MainDocAMKA**) performed within the current year. This composite index isolates the doctor first (an attribute with high selectivity and cardinality) and then sorts by DateTime. This allows the database to execute an "Index Range Scan", skipping the data blocks of procedures performed in previous years entirely.
...
> To trace a doctor's hierarchy up to the Director, the database must use a Recursive Common Table Expression. Internally, a recursive CTE performs an iterative self-join on the Doctor table. If the `SupervisorAMKA` is not indexed, every single iteration (step up the hierarchy) forces a full table scan. This index guarantees logarithmic time complexity O(logn) for hierarchical traversals.
...
| `trg_hosp_eval_insert` | Ensure `HospEvaluation` is only inserted after the patient has been discharged. |
| `trg_hosp_eval_update` | Ensure `HospEvaluation` updates are only allowed if the patient has been discharged. |
| `trg_doc_eval_insert` | Ensure `DoctorEvaluation` is only inserted after discharge and only for a doctor who prescribed during that hospitalization. |
| `trg_doc_eval_update` | Ensure `DoctorEvaluation` updates preserve the discharge and doctor-prescription rules. |
